...
public class Alejandro {
    ...
    private static void resolver(Graph<Nodo, DefaultWeightedEdge> grafo) {
        ...
        while(!nodoEncontrado && !maximoLocal) {
            if(nodoActual == null) {
                ...
                //esto representa si avanzamos a otro nodo
                nodoAux = nodoActual;
                for(int i=0;i<adyacentes.size();i++) {
                    //agarro los nodos adyacentes que no visite
                    if(nodoActual.getValor() < adyacentes.get(i).getValor() && !visitados.contains((Nodo) adyacentes.get(i))) {
